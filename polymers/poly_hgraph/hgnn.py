...
def make_cuda(tensors):
    ...
    tree_tensors = [make_tensor(x).cuda().long() for x in tree_tensors[:-1]] + [
        tree_tensors[-1]
    ]
...
class HierVGNN(nn.Module):
    ...
    def translate(self, tensors, num_decode, enum_root, greedy=True):
        ...
        z_tree = (
            torch.randn(batch_size, 1, self.latent_size)
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, beta):
        ...
        diff_graph_vecs, graph_kl = self.rsample(
            diff_graph_vecs, self.G_mean, self.G_var
        )
...
class HierCondVGNN(HierVGNN):
    def __init__(self, args):
        ...
        self.W_tree = nn.Sequential(
            nn.Linear(
                args.hidden_size + args.latent_size + args.cond_size, args.hidden_size
            ),
            nn.ReLU(),
        )
        self.W_graph = nn.Sequential(
    ...
    def translate(self, tensors, cond, num_decode, enum_root):
        ...
        z_graph_vecs = self.W_graph(
            torch.cat([graph_vecs, z_graph, graph_cond], dim=-1)
        )
