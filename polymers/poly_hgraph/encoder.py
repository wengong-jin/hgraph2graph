...
class HierMPNEncoder(nn.Module):
    def __init__(
        self, vocab, avocab, rnn_type, embed_size, hidden_size, depthT, depthG, dropout
    ):
        ...
        self.graph_encoder = MPNEncoder(
            rnn_type, atom_size + bond_size, atom_size, hidden_size, depthG, dropout
        )
...
class IncHierMPNEncoder(HierMPNEncoder):
    ...
    def get_sub_tensor(self, tensors, subset):
        ...
        agraph, bgraph = (
            agraph.index_select(0, subnode),
            bgraph.index_select(0, submess),
        )
