...
class MolGraph(object):
    ...
    def tree_decomp(self):
        ...
        for atom, nei_cls in enumerate(self.atom_cls):
            ...
            if (
                len(nei_cls) > 2 and len(inter) == 1
            ):  # two rings + one bond has problem!
    ...
    def pool_clusters(self):
        ...
        for fsmiles, fatoms in fragments:
            if fsmiles not in MolGraph.FRAGMENTS:
                continue
        ...
        hoptions = sorted(
            hoptions, key=lambda x: min(x)
        )  # to ensure hoptions[0] has the root node
