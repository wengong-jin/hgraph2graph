...
class IncBase(object):
    def __init__(
        self, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=200, max_nb=12
    ):
        self.max_nb = max_nb
...
class IncTree(IncBase):
    def __init__(
        ...
        super(IncTree, self).__init__(
            batch_size, node_fdim, edge_fdim, max_nodes, max_edges, max_nb
        )
...
class IncGraph(IncBase):
    ...
    def get_assm_cands(self, cluster, used, smiles):
        ...
        if emol.GetNumAtoms() == 1:
            ...
            attach_points = [
                atom.GetIdx() for atom in emol.GetAtoms() if atom.GetAtomMapNum() == 1
            ]
